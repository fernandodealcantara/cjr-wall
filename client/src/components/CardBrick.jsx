...
export default function CardBrick({ color, picture, name, userId }) {
  return (
    <Link to={`profile/${userId}`}>
      <div
        className={`flex items-center pl-6 bg-center bg-no-repeat min-h-[86px] 
                    min-w-[244px] max-h-[86px] max-w-[244px] rounded ${color}
                    ease-in-out cursor-pointer transition transform hover:scale-105 hover:shadow-2xl`}
      >
        <img
